  ...
  try {
    ...
    if (isGoogleModel(model)) {
      ...
      if (isImagenModel) {
        ...
        console.log(`[Imagen] Generating image with model ${actualModel}, prompt: ${prompt.substring(0, 50)}...`);
        ...
        console.log(`[Gemini] Generating image with model ${actualModel}, prompt: ${prompt.substring(0, 50)}...`);
        ...
        if (imageUrls.length === 0) {
          console.error("[Gemini] No images in response:", JSON.stringify(data, null, 2).substring(0, 1000));
          return NextResponse.json(
            { error: "No images generated by Gemini", details: JSON.stringify(data).substring(0, 500) },
            { status: 500 }
          );
        }
    ...
    console.log(`[Replicate] Generating image with model ${model}, prompt: ${prompt.substring(0, 50)}...`);
