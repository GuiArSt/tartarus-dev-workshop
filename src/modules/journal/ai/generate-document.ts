...
export const DocumentAgentInputSchema = z.object({
  raw_agent_report: z.string().min(10).describe('Agent\'s full report describing the document or prompt to create'),
...
export const DocumentAIOutputSchema = z.object({
  ...
  content: z.string().min(1).describe('Document content (for prompts, this is the prompt text)'),
  type: z.enum(['writing', 'prompt', 'note']).describe('Document type (writing, prompt, or note)'),
  ...
  metadataType: z.string().optional().describe('Secondary category (metadata.type) - free-form string'),
  ...
  inputSchema: z.string().optional().describe('For prompts: JSON schema for input validation (as JSON string)'),
  outputSchema: z.string().optional().describe('For prompts: JSON schema for expected output (as JSON string)'),
...
3. **Extract content** - the full text of the document/prompt
4. **Extract metadata** - tags, categories, dates mentioned in the report
5. **For prompts**: Extract purpose, role, schemas, and config if mentioned
